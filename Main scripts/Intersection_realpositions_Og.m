...
%hulls, in their real positions in the region.
%Version 1.0
...
Intersection_realpositions_Og_T = [];          %%%Name of the output matrix that contains data analyses.
...
for c = 1:sCorr
    ...
    %%%Add cell body position (comment if neuron position was considered in ConvexHull2LinePoints)
    
    Hull_1 = Hull_1(:,1:3)+xyz_position(nCorr(c,1),:);           
    Hull_2 = Hull_2(:,1:3)+xyz_position(nCorr(c,2),:); 
    ...
    Intersection_realpositions_Og_T = [Intersection_realpositions_Og_T; nCorr(c,1) nCorr(c,2) ...
        xyz_position(nCorr(c,1),:) xyz_position(nCorr(c,2),:) d v1 v2 v3 v3/(v1+v2-v3)*100];
   
    cd(folder);
    save('Intersection_realpositions_Og','Intersection_realpositions_Og')
